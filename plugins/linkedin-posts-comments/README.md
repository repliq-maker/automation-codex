...
If Codex Settings shows the plugin and MCP as installed but a chat still says the `linkedin-posts-comments` skill or Apify tools are missing, that chat did not receive the runtime skill/tool injection. Use the robust runtime prompt in `DAILY_AUTOMATION_GUIDE.md`; it tells the agent to read the cached skill file manually and use Apify tool discovery before reporting a Codex runtime loading blocker.
...
The setup prompt is intentionally one prompt with two passes. Pass 1 installs/connects all needed tools, including the official Google Drive plugin/connector, and may require a full Codex restart. Pass 2, after fully quitting/reopening Codex, can continue in the same setup chat with `continue` or in a new chat with the same prompt. The prompt is idempotent, so a new chat should detect already-correct marketplace/plugin/cache/MCP state and continue to verification/Sheet setup instead of reinstalling tools or asking for another restart. If the custom skill or Apify tools still are not visible but config/cache/MCP are correct, it should create the Sheet anyway and report `SETUP SHEET READY, RUNTIME LOAD CHECK BLOCKED` instead of looping through more new chats. That blocked-state output should include a small copy/paste runtime test prompt with cached-skill and Apify tool-discovery fallback instructions. Users should not run the automation until Pass 2 says `READY TO RUN`.
...
3. After the full Codex restart, type `continue` in the same setup chat or paste the same setup prompt in a new chat so it can create the Sheet, tab, and headers. Folder placement is optional/manual when the connector cannot create folders or move files. If setup reports `SETUP SHEET READY, RUNTIME LOAD CHECK BLOCKED`, stop repeating setup and use a normal run chat as the runtime diagnostic.
