...
If the setup prompt changes marketplace, plugin, MCP, or connector state, fully quit and reopen Codex. Then either type `continue` in the same setup chat or open a new chat and paste the same setup prompt again. The prompt is idempotent: a new chat should detect already-correct marketplace/plugin/cache/MCP state and continue to verification/Sheet setup instead of reinstalling tools or asking for another restart. If the custom skill or Apify tools still are not visible but config/cache/MCP are correct, setup should create the Sheet anyway and report `SETUP SHEET READY, RUNTIME LOAD CHECK BLOCKED` instead of asking for endless new chats. That blocked-state output should include a small copy/paste runtime test prompt with cached-skill and Apify tool-discovery fallback instructions. Do not run the daily automation until setup says `READY TO RUN`; the final readiness pass should include a real lightweight Apify smoke test, not only saved-config checks.
